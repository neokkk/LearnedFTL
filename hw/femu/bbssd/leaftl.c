...
/* process hash */
static inline uint64_t cmt_hash(uint64_t lpn) { return lpn % CMT_HASH_SIZE; }
...
static inline void set_gtd_ent(struct ssd *ssd, uint64_t tvpn,
                               struct ppa *ppa) {
...
static uint64_t ppa2vppn(struct ssd *ssd, struct ppa *ppa) {
  struct ssdparams *spp = &ssd->sp;
  uint64_t vppn;
  vppn = ppa->g.ch + ppa->g.lun * spp->chn_per_lun +
         ppa->g.pl * spp->chn_per_pl + ppa->g.pg * spp->chn_per_pg +
    ...
    if (twpp->lun == spp->luns_per_ch) {
      ...
      if (twpp->pg == spp->pgs_per_blk) {
        ...
        if (twpp->curline->vpc == spp->pgs_per_line) {
          ...
          ftl_assert(twpp->curline->vpc >= 0 &&
                     twpp->curline->vpc < spp->pgs_per_line);
          /* there must be some invalid pages in this line */
          ftl_assert(twpp->curline->ipc > 0);
...
static uint64_t translation_page_read(struct ssd *ssd, NvmeRequest *req,
                                      struct ppa *ppa) {
  uint64_t lat = 0;
  struct nand_cmd trd;
  trd.type = USER_IO;
...
static inline uint64_t translation_page_read_no_req(struct ssd *ssd,
                                                    struct ppa *ppa) {
  uint64_t lat = 0;
  struct nand_cmd trd;
  trd.type = USER_IO;
...
//     /* after reading the translation page, new translation page can begin to
//     write */
//     // old_lun = get_lun(ssd, old_ppa);
//     // new_lun = get_lun(ssd, &new_ppa);
//     // new_lun->next_lun_avail_time = (old_lun->next_lun_avail_time >
//     new_lun->next_lun_avail_time) ?
//     //                             old_lun->next_lun_avail_time :
//     new_lun->next_lun_avail_time;
//     /* update maptbl */
...
static uint64_t translation_page_write_via_tvpn(struct ssd *ssd,
                                                struct ppa *old_ppa,
                                                uint64_t tvpn) {
  struct ppa new_ppa;
  // struct nand_lun *new_lun, *old_lun;
...
static void insert_segment_to_cmt(struct ssd *ssd, uint64_t lpn,
                                  simple_segment sm) {
  struct cmt_mgmt *cm = &ssd->cm;
  struct hash_table *ht = &cm->ht;
  struct cmt_entry *cmt_entry;
...
static struct nand_lun *process_translation_page_read(struct ssd *ssd,
                                                      NvmeRequest *req,
                                                      uint64_t lpn,
                                                      uint64_t *ppn) {
  struct ssdparams *spp = &ssd->sp;
...
static uint64_t gc_translation_page_write(struct ssd *ssd,
                                          struct ppa *old_ppa) {
  struct ppa new_ppa;
  struct nand_lun *new_lun;
  uint64_t tvpn = get_rmap_ent(ssd, old_ppa);
...
static void clean_one_data_block(struct ssd *ssd, struct ppa *ppa,
                                 uint64_t *valid_lpns, int *pointer) {
  struct ssdparams *spp = &ssd->sp;
...
static void regression(simple_segment *sg, uint64_t *lpns, uint64_t *ppns,
                       int len, learned_mgmt *lm, int seg_no) {
  float k, b;
...
static int segment_search_simple_segment(simple_segment *sg, uint64_t lpn,
                                         uint64_t *ppn, learned_mgmt *lm,
...
static segment *
segment_insert_simple_segment(segment *sg, simple_segment *new_seg, int len) {
...
static void get_valid_lpns(segment *seg, uint8_t *valid_lpns, int seg_no,
                           segment_param *sp, learned_mgmt *lm) {
  simple_segment *sm = seg->segment_head;
...
static int level_seg_mgmt_search(segment *seg, uint64_t lpn, uint64_t *ppn,
                                 segment_param *sp, learned_mgmt *lm,
                                 simple_segment *lru_en) {
  int seg_no = lpn / sp->mapping_per_page;
  if (seg == NULL) {
...
static int level_seg_mgmt_insert(level_seg_mgmt *lsegm, uint64_t *lpns,
                                 uint64_t *ppns, int len, segment_param *sp,
                                 learned_mgmt *lm) {
...
static void all_seg_mgmt_insert_mappings(all_seg_mgmt *all_segs,
                                         segment_param *sp, uint64_t *lpns,
                                         uint64_t *ppns, int len,
                                         learned_mgmt *lm) {
  level_seg_mgmt_insert(all_segs->levels, lpns, ppns, len, sp, lm);
}

static uint64_t all_seg_get_ppn(all_seg_mgmt *all_segs, segment_param *sp,
                                uint64_t lpn, learned_mgmt *lm,
                                simple_segment *lru_en) {
  uint64_t ppn = 0;
...
static void delete_seg(learned_mgmt *lm, segment_param *sp, segment *seg,
                       int seg_no, int *delete_cnt) {
  simple_segment *sm = seg->segment_head;
  conflict_resolution_buffer *crb = lm->crb;
  simple_segment *tmp;
...
static void flush_lpn_ppn_mappings(learned_mgmt *lm, uint64_t *lpns,
                                   uint64_t *ppns, int len) {
  all_seg_mgmt_insert_mappings(lm->all_segs, lm->sp, lpns, ppns, len, lm);
}

static uint64_t get_ppn(learned_mgmt *lm, uint64_t lpn,
                        simple_segment *lru_en) {
...
static void gc_flush(struct ssd *ssd, uint64_t *buffer_lpns, int pointer) {
  ...
  for (int i = 0; i < buffer_size; i++) {
    ...
    if (i > 0) {
      if (lpns[i] - lpns[i - 1] > threshold ||
          (i - last_pointer >= TRAIN_LEN) || i == buffer_size - 1) {
        ...
        // all_seg_mgmt_insert_mappings(all_segs, sp, new_lpn, new_ppn, len,
        // lm);
...
static uint64_t buffer_flush(struct ssd *ssd, NvmeRequest *req) {
  ...
  for (int i = 0; i < buffer_size; i++) {
    ...
    if (i > 0) {
      if (lpns[i] - lpns[i - 1] > threshold ||
          (i - last_pointer >= TRAIN_LEN) || i == buffer_size - 1 ||
          lpns[i] / lm->sp->mapping_per_page !=
              lpns[i - 1] / lm->sp->mapping_per_page) {
        // * start builing one segment
        int len = i - last_pointer;
...
static bool is_same_segment(simple_segment *s1, simple_segment *s2) {
  return s1->inter == s2->inter && s2->k == s1->k &&
         s1->is_accurate == s2->is_accurate && s1->len == s2->len &&
         s1->slpa == s2->slpa;
}
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req) {
  ...
  for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
    ...
    if (cmt_ent) {
      ...
      segment_search_simple_segment(&cmt_ent->sm, lpn, &vppn, ssd->lem,
                                    &lru_en);
      ...
      if (!is_same_segment(&lru_en, &cmt_ent->sm)) {
        ...
        if (!mapped_ppa(&ppa) || !valid_ppa(ssd, &ppa)) {
          // printf("%s,lpn(%" PRId64 ") not mapped to valid ppa\n",
          // ssd->ssdname, lpn); printf("Invalid
          // ppa,ch:%d,lun:%d,blk:%d,pl:%d,pg:%d,sec:%d\n", ppa.g.ch, ppa.g.lun,
          // ppa.g.blk, ppa.g.pl, ppa.g.pg, ppa.g.sec);
          continue;
        }
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req) {
  ...
  for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
    ...
    if (ssd->lem->bitmap[lpn]) { // nk: if approximate segment
    ...
    if (st->write_cnt % 1000000 == 0) { // nk: ???
      compact(ssd, ssd->lem);

      // TODO: compact结束之后执行GC
    }

    for (int i = 0; i < ssd->sp.buffer_size; i++) { // nk: O(n^2) 실화?
      if (ssd->buffer_lpns[i] == lpn) {
...
static void *ftl_thread(void *arg) {
  ...
  gc_fp =
      fopen("/home/lzh/femu/log/gc_frequency.txt", "w"); // TODO: adapt user pwd
